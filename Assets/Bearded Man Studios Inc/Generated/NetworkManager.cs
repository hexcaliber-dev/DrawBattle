...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] DrawableTextureNetworkObject = null;
		public GameObject[] ExampleProximityPlayerNetworkObject = null;
		public GameObject[] HomeBaseNetworkObject = null;
		...
		public GameObject[] TimerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			if (obj is BarrierBlockNetworkObject)
			{
			...
			else if (obj is BattleNetworkNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
			...
			else if (obj is DrawableTextureNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DrawableTextureNetworkObject.Length > 0 && DrawableTextureNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DrawableTextureNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DrawableTextureBehavior>();
						}
					}
			...
			else if (obj is HomeBaseNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (HomeBaseNetworkObject.Length > 0 && HomeBaseNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(HomeBaseNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<HomeBaseBehavior>();
						}
					}
			...
			else if (obj is TimerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (TimerNetworkObject.Length > 0 && TimerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(TimerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<TimerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDrawableTexture instead, its shorter and easier to type out ;)")]
		public DrawableTextureBehavior InstantiateDrawableTextureNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DrawableTextureNetworkObject[index]);
			var netBehavior = go.GetComponent<DrawableTextureBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DrawableTextureBehavior>().networkObject = (DrawableTextureNetworkObject)obj;
		...
		[Obsolete("Use InstantiateHomeBase instead, its shorter and easier to type out ;)")]
		public HomeBaseBehavior InstantiateHomeBaseNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(HomeBaseNetworkObject[index]);
			var netBehavior = go.GetComponent<HomeBaseBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<HomeBaseBehavior>().networkObject = (HomeBaseNetworkObject)obj;
		...
		[Obsolete("Use InstantiateTimer instead, its shorter and easier to type out ;)")]
		public TimerBehavior InstantiateTimerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TimerNetworkObject[index]);
			var netBehavior = go.GetComponent<TimerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<TimerBehavior>().networkObject = (TimerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of DrawableTexture
		/// </summary>
		/// <returns>
		/// A local instance of DrawableTextureBehavior
		/// </returns>
		/// <param name="index">The index of the DrawableTexture prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public DrawableTextureBehavior InstantiateDrawableTexture(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DrawableTextureNetworkObject[index]);
			var netBehavior = go.GetComponent<DrawableTextureBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of HomeBase
		/// </summary>
		/// <returns>
		/// A local instance of HomeBaseBehavior
		/// </returns>
		/// <param name="index">The index of the HomeBase prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public HomeBaseBehavior InstantiateHomeBase(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(HomeBaseNetworkObject[index]);
			var netBehavior = go.GetComponent<HomeBaseBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Timer
		/// </summary>
		/// <returns>
		/// A local instance of TimerBehavior
		/// </returns>
		/// <param name="index">The index of the Timer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public TimerBehavior InstantiateTimer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TimerNetworkObject[index]);
			var netBehavior = go.GetComponent<TimerBehavior>();
